...
\item Rotational frequency of at least \SI{30}{\hertz}, or as fast as it needs to be in order
	to not flicker
...
some power. The processor runs at \SI{96}{\mega\hertz} at a register width of 32 bit, and there
are 34 configurable I/O pins.
...
moving into a new pixel 3000 times a second (assuming rotation at 30 Hz). This means that a PWM
...
for example 10, which would require a PWM frequency of \SI{30}{\kilo\hertz}.
...
the next larger distance is \SI{125}{mil}, which is equal to \SI{3.175}{\milli\metre}.
...
connection to have too much parasitic capacitance, and the signals (which were being sent at
\SI{4}{\mega\hertz}) did not have time to propagate up to their destination.
...
% TODO put source code in appendix
